...
class Data_model extends CI_Model
{
	...
	public function kyc_verified_data($limits, $starts, $dfilter, $ffilter, $kycfilter, $fworker_type, $fjob, $print = FALSE)
	{
		if ($print == 1) {
			$limit = "";
		} else {
		...
		$query = $this->db->query("SELECT v.*,r.birth_date,r.district,r.facility,r.hw_type,r.job,r.national_id FROM validated_numbers v JOIN records_json_report r ON v.reference=r.reference and status in 'MATCH,CLOSE MATCH,POSSIBLE MATCH,VERIFIED MATCH' $dfilter $kycstatus $ffilter $kycfilter $fworker_type $fjob $limit");
		return $query->result();
	}
